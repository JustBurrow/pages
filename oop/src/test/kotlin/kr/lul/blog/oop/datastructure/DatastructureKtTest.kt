...
class DatastructureKtTest : BehaviorSpec({
    val log = KotlinLogging.logger { }

    given("현재 버전이 1.2.3 이고") {
        val current = convert("1.2.3")
        log.info("[GIVEN] current=$current")

        `when`("필요 버전이 1.2.0 이면") {
            val required = convert("1.2.0")
